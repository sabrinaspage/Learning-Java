...
public class MyArc extends MyShape {
	...
	public MyArc(MyPoint ref, MyColor color, MyOval basis, MyPoint coordinate, double startAngle, double probability,
			String text, int i) {
		super(ref, color);
		this.basis = basis;
	...
	public void draw(GraphicsContext gc) {
		...
		gc.setLineWidth(1);
		gc.fillText(text, 330, 30 * i + 30);
		gc.strokeArc(coordinate.getX(), coordinate.getY(), basis.getWidth() / 2, basis.getHeight() / 2, this.startAngle,
				this.probability, ArcType.ROUND);
		gc.fillArc(coordinate.getX(), coordinate.getY(), basis.getWidth() / 2, basis.getHeight() / 2, this.startAngle,
				this.probability, ArcType.ROUND);
	}
	...
	public String toString() {
		...
		return String.format(
				"Coordinate for angle: (%s, %s); \nWidth: %s \nHeight: %s \nArc starting angle: %s \nArc spacing: %s \nThe color: %s\n",
				dec.format(coordinate.getX()), dec.format(coordinate.getY()), basis.getWidth() / 2,
				basis.getHeight() / 2, dec.format(this.startAngle), dec.format(this.probability),
